// FRACTURED Angel Knight frame renderer v7
const FRAME_ROOT="./angel_frames";
export const SPRITE_ANIMS={
  idle:{dir:"idle",frames:4,fps:5,loop:true},
...
const frameUrl=(dir,i)=>`${FRAME_ROOT}/${dir}/${i}.png?v=7`;
export class AngelKnightSpriteRenderer{
  constructor(){
    this.images={};this.state="idle";this.frame=0;this.time=0;
    for(const [state,cfg] of Object.entries(SPRITE_ANIMS)){
      this.images[state]=[];
      for(let i=0;i<cfg.frames;i++){const img=new Image();img.src=frameUrl(cfg.dir,i);this.images[state].push(img)}
    }
  }
  ...
  update(dt,eventHandler){
    ...
    const fd=1/cfg.fps;
    while(this.time>=fd){
      this.time-=fd;this.frame++;
      if(this.frame>=cfg.frames){
        if(cfg.loop)this.frame=0;
        else{this.frame=cfg.frames-1;return "finished";}
      }
      if(this.state==="attack1"&&this.frame===2)eventHandler?.("hit",{hitbox:{x:34,y:-92,w:118,h:76,damage:20,knockback:220}});
      if(this.state==="attack2"&&this.frame===2)eventHandler?.("hit",{hitbox:{x:28,y:-105,w:135,h:86,damage:25,knockback:260}});
      if(this.state==="attack3"&&this.frame===2)eventHandler?.("hit",{hitbox:{x:24,y:-118,w:155,h:100,damage:36,knockback:330}});
