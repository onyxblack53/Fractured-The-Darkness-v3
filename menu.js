...
function totalStats(){const o={...baseStats};for(const k in equipment){const it=ItemDB[equipment[k]];if(!it?.stats)continue;for(const n in it.stats){const v=it.stats[n];if(typeof v==="number"&&typeof o[n]==="number")o[n]+=v}}return o}
function itemCard(id){const i=ItemDB[id];return `<button class="itemCard ${selectedItem===id?"selected":""}" data-item="${id}"><span>${i.icon}</span>${i.qty?`<b>${i.qty}</b>`:""}</button>`}
function equipSlot(slot,label){const id=equipment[slot],i=id&&ItemDB[id];return `<button class="equipSlot"><span class="equipIcon">${i?i.icon:"+"}</span><span><small>${label}</small><strong>${i?i.name:"Empty"}</strong></span></button>`}
...
function renderInventory(){
 ...
 document.querySelector("#itemDetail").innerHTML=`<div class="itemHero">${i.icon}</div><h2>${i.name}</h2><em>${i.rarity}</em><div class="itemStats">${Object.entries(i.stats||{}).map(([k,v])=>`<div><span>${k}</span><b>${v}</b></div>`).join("")}</div><p>${i.desc||"An item recovered from the fractured realms."}</p>`;
 document.querySelectorAll("[data-item]").forEach(b=>b.onclick=()=>{selectedItem=b.dataset.item;renderInventory()});
}

function renderCharacter(){
 const build=currentBuild(),st=totalStats();
 document.querySelector("#leftEquip").innerHTML=equipSlot("head","HEAD")+equipSlot("chest","CHEST")+equipSlot("arms","ARMS")+equipSlot("legs","LEGS")+equipSlot("main","MAIN HAND")+equipSlot("off","OFF HAND");
 document.querySelector("#rightEquip").innerHTML=equipSlot("amulet","AMULET")+equipSlot("ring1","RING 1")+equipSlot("ring2","RING 2")+equipSlot("relic","RELIC")+equipSlot("consumable","CONSUMABLE")+equipSlot("quick","QUICK ITEM");
 const groups=[["CORE STATS",["Health","Mana","Stamina","Strength","Dexterity","Intelligence","Faith","Vitality"]],["COMBAT STATS",["Attack","Magic Attack","Defense","Magic Defense","Critical Chance","Critical Damage","Attack Speed","Movement Speed"]],["RESISTANCES",["Physical","Fire","Ice","Lightning","Holy","Dark","Poison","Bleed"]]];
 const res={Physical:"32%",Fire:"18%",Ice:"18%",Lightning:"18%",Holy:hasRace("Angel")?"42%":"18%",Dark:"10%",Poison:"12%",Bleed:"14%"};
 document.querySelector("#statColumns").innerHTML=groups.map(([title,names])=>`<section><h3>${title}</h3>${names.map(n=>`<div><span>${n}</span><b>${st[n]??res[n]??0}</b></div>`).join("")}</section>`).join("");
